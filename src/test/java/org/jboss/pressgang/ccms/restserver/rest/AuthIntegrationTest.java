...
public class AuthIntegrationTest extends BaseArquillianIntegrationTest {
    ...
    public void shouldGetResourceMatchingIdWithJsonp() {
        ...
        String result = given().header("Authorization", "Bearer access_token")
                .expect().statusCode(200)
                .when().get(getBaseTestUrl() + "/1/category/get/jsonp/1?callback=functionx")
                .asString();
    ...
    public void shouldGetExpandedResource() {
        ...
        given().header("Authorization", "Bearer access_token")
                .expect().statusCode(200)
                .and().expect().body("items.item.name", hasItems("Audiences", "Concerns", "Technologies"))
                .and().expect().body("items.item.sort", hasItems(15, 25, 20))
                .when().get(getBaseTestUrl() + "/1/categories/get/json/all?expand=" +
    ...
    public void shouldGetResourceByQuery() {
        ...
        given().header("Authorization", "Bearer access_token")
                .expect().statusCode(200)
                .and().expect().body("size", equalTo(1))
                .and().expect().body("items.item.name", hasItem("Technologies"))
                .and().expect().body("items.item.sort", hasItem(20))
                .when().get(getBaseTestUrl() + "/1/categories/get/json/query;catName=Technologies?expand=" +
                "{\"branches\":[{\"trunk\":{\"name\":\"categories\"}}]})");
    }
    ...
    public void shouldDeleteResourcesByIds() {
        ...
        given().header("Authorization", "Bearer access_token")
                .expect().statusCode(200)
                .when().delete(getBaseTestUrl() + "/1/tags/delete/json/ids;1;3;");
        // Then then the resources should be deleted
        given().header("Authorization", "Bearer access_token")
                .expect().body("size", equalTo(1))
                .and().expect().body("items.item.name", hasItem("Concept"))
                .and().expect().body("items.item.name", not(hasItem("Task")))
                .and().expect().body("items.item.name", not(hasItem("Reference")))
                .when().get(getBaseTestUrl() + "/1/tags/get/json/all?expand=" +
    ...
    public void shouldFailToGetResourceWhenIncorrectScopeForEndpoint() {
        ...
        given().header("Authorization", "Bearer access_token")
                ...
                .when().delete(getBaseTestUrl() + "/1/category/delete/json/1");
    }
    ...
    public void shouldFailToGetResourceWhenHttpMethodIncorrect() {
        ...
        given().header("Authorization", "Bearer access_token")
                ...
                .when().put(getBaseTestUrl() + "/1/category/get/json/1");
    }
    ...
    public void shouldFailToGetResourceWhenTokenNonCurrent() {
        ...
        // When the request is made
        // Then an authorization error should be returned
        given().header("Authorization", "Bearer noncurrent_access_token")
                .expect().statusCode(401)
                .when().get(getBaseTestUrl() + "/1/category/get/json/1");
    }
