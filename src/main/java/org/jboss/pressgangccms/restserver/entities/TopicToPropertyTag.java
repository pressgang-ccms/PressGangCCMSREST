	...
	protected boolean testUnique(final EntityManager entityManager, final Number revision)
	{
		try
		{
			if (this.propertyTag.getPropertyTagIsUnique())
			{
				...
				if (revision == null)
				{
					final String query = TopicToPropertyTag.SELECT_SIZE_QUERY + " WHERE topicToPropertyTag.propertyTag = " + this.propertyTag.getId() + " AND topicToPropertyTag.value = '" + this.getValue() + "'";
